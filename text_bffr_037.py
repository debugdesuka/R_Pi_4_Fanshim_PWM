...
from bffr_log_037 import class_bffr_log
...
class class_text_bffr(object):
	...
	def __init__(self, size_max, headings,name,config,log_bffr_flag):
		#initialization
		...
		self.line_values = ["Text"]*len(headings)
		...
		if log_bffr_flag:
			self.__log_bffr_flag = True
			self.__send_log_count = 0
			self.__log = class_bffr_log(name,config)
		else:
			self.__log_bffr_flag = False

#	def set_bffr_filenames(self, html_filename,html_filename_save_as,www_filename,ftp_creds):
...
#	def set_bffr_headings(self,headings):
#		print("first and last",headings[0],headings[self.__width-2])
#		for hdg_ind in range(0,self.__width-1):
#			print(hdg_ind,headings[hdg_ind])
#			self.__headings.append(headings[hdg_ind])
#		print(self.__headings)
#		test_cpu_bffr_width = len(self.__headings)
#		print("width check",self.__width,test_cpu_bffr_width)
	...
	def update_bffr(self,values,appnd,ref):
		...
		#print("Buffer updated and log buffer flag is : ",self.__log_bffr_flag)
		if self.__log_bffr_flag:
	...
	def pr(self,appnd,ref,log_time):
		here = "bffr.pr for " + self.__name
...
<tbody>"""
		...
		try:
			...
			print("Error in make values in ...bffr.pr for : ",self.__name)
